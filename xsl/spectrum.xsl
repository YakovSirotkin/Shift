...
<head>
        ...
        {font-weight:bold;};
            ...
            <body>
                ...
                <script>
                    ...
                    var br = "&lt;br&gt;\r";
                    function renderMatch(proteinId, protein, shift) {
                        for (var i = 0;  peaks.length > i; i++) {
                            peaks[i].selected = false;
                        }
                        score = 0;
                        ...
                        for (var cur = 0; protein.length > cur; cur++) {
                            ...
                            var add = "&#160;";
                            if (targetMass > - 0.1 &amp;&amp; total + 0.1 > targetMass) {
                                var count = 0;
                                for (var i = 0;  peaks.length > i; i++) {
                                    var p = peaks[i];
                                    if (!p.selected) {
                                ...
                                if (count > 0) {
                                    add = "&lt;a href='#' title='" + count + "'&gt;";
                                    add +=  "|";
                                    add += "&lt;/a&gt;"
                                }
                                text += add + protein.charAt(cur);
                            } else {
                                text += "&lt;font color='grey'>" + add + protein.charAt(cur) + "&lt;/font>";
                            }

                            targetMass += acidMass(protein.charAt(cur));
                        }
                        text +=  br + br;
                        text += "There are " + score + " matched peaks.";
    ...
    <xsl:template match="match">
        <h3>Match with protein #<xsl:value-of select="protein-id"/></h3>
        <p id="p{protein-id}" style="font-family: 'FreeMonoMedium', FreeMonoMedium, Miltonian, monospace;"></p>
        <script>
            renderMatch( <xsl:value-of select="protein-id"/>, '<xsl:value-of select="protein"/>', <xsl:value-of select="shift"/>);
        </script>
        <xsl:apply-templates select="shifts/shift"/>
    </xsl:template>

    <xsl:template match="shift">
        <a href="#" onclick="renderMatch({../../protein-id}, '{../../protein}', {value});">
            <xsl:value-of   select="value"/>(<xsl:value-of select="score"/>)</a>&#160;
    </xsl:template>
